...
video-registry is developed with:
...
type: ::
    ...
    git clone https://github.com/sveetch/video-registry.git
    cd video-registry
    make install

video-registry will be installed in editable mode from the
...
Then go in the ``video-registry`` directory, where the
